from data_filtering.data_loader import *  # Make sure this import is correct and available
...
class HousingDataProcessor:
    ...
    def process_decade_data(self, df, years_label):
        ...
        for location, location_df in df.groupby('Neighborhood'):
            ...
            for (bedrooms, bathrooms), bed_bath_df in location_df.groupby(['Bedrooms', 'Bathrooms']):
                ...
                grouped_df['YearBuilt'] = bed_bath_df['YearBuilt'].iloc[0]
    ...
    def remove_unrealistic_prices(self, df):
        ...
        # Usuwanie wierszy na podstawie cen, liczby łazienek i sypialni
        df = self.compare_prices_by_rooms(df)
        return df

    def compare_prices_by_rooms(self, df):
        # Sort by Neighborhood, SquareFeet, and Bedrooms
        df_sorted = df.sort_values(by=['Neighborhood', 'AverageSquareFeet', 'Bedrooms'])

        to_drop = []

        # Group by Neighborhood and AverageSquareFeet
        grouped = df_sorted.groupby(['Neighborhood', 'AverageSquareFeet'])

        for (neighborhood, square_feet), group in grouped:
            if len(group) > 1:
                for i in range(len(group) - 1):
                    current_row = group.iloc[i]
                    next_row = group.iloc[i + 1]

                    # Check if the square feet difference is <= 50 and compare prices
                    if abs(current_row['AverageSquareFeet'] - next_row['AverageSquareFeet']) <= 50:
                        if current_row['Bedrooms'] < next_row['Bedrooms'] and current_row['AveragePrice'] < next_row['AveragePrice']:
                            to_drop.append(next_row.name)  # Mark for removal
...
# Przykład użycia:
processor = HousingDataProcessor(df, start_year=1950, end_year=1959)
wynik_df = processor.process_all()
processor.save_result("wynik3.csv")
file_directory = r"wynik3.csv"
df = pd.read_csv(file_directory)
print(df[(df['Neighborhood'] == 'Rural') & (df['Bedrooms'] == 2) & (df['Bathrooms'] == 1)].sort_values(by='YearBuilt'))
